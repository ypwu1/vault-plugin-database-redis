...
while [[ "$(curl -k -s -o /dev/null -w '%{http_code}' https://${MINIKUBE_IP}:9443/v1/bootstrap)" != "200" ]]; do
...
jq --null-input --arg username "$username" --arg password "$password" \
  ...
  curl -fks -X POST "https://${MINIKUBE_IP}:9443/v1/bootstrap/create_cluster" -H "Content-Type: application/json" -H  "accept: application/json" -d @- -o /dev/null

while [ "$(curl -fks -u "$username:$password" https://${MINIKUBE_IP}:9443/v1/bootstrap | jq -r .bootstrap_status.state)" != "completed" ]; do
  echo "waiting for cluster bootstrap"
  sleep 1;
done

# Create the initial database and wait for it to finish being set up
acl_id=$(curl -fks -u "$username:$password" https://${MINIKUBE_IP}:9443/v1/redis_acls | jq '.[] | select(.name=="Not Dangerous").uid')
role_id=$(curl -fks -u "$username:$password" https://${MINIKUBE_IP}:9443/v1/roles | jq '.[] | select(.name=="DB Member").uid')
jq --null-input --arg acl_id "$acl_id" --arg role_id "$role_id" --arg db_name "$db_name" --arg password "$password" \
  ...
  curl -fks -u "$username:$password" -X POST -H "Content-Type: application/json" -d @- https://${MINIKUBE_IP}:9443/v1/bdbs -o /dev/null

while [ "$(curl -fks -u "$username:$password" https://${MINIKUBE_IP}:9443/v1/bdbs | jq --arg db_name "${db_name}" -r '.[] | select(.name == $db_name) | .status')" != "active" ]; do
